...
class SimpleMap extends Component {
  render() {
    ...
    if (countries.length > 0) {
      ...
      return (
        <Fragment>
          <ComposableMap data-tip="" projectionConfig={{ scale: 170 }} style={{ width: "100%", backgroundColor: "#FFFFFF" }}>
            <Geographies geography={worldGeoData} fill="#DDDDDD" stroke="#FFFFFF" strokeWidth="1">
              {({ geographies }) =>
                geographies.map(geo => (
          ...
          { tooltip[0] !== '' && tooltip[0] !== undefined && tooltip[0] !== null && <ReactTooltip>{tooltip[0]} – <NumberFormat value={tooltip[1]} displayType={'text'} thousandSeparator={true} /></ReactTooltip>}
        </Fragment>
      );
    } else {
     return (
